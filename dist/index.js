...
slider.checkButtons = function() {
    (0, _utilsJs.checkButtonState)(this.sliderInner, this.prevButton, this.nextButton);
};
slider.addListeners = function() {
    if (this.prevButton) this.prevButton.addEventListener('click', this.moveToPrevious.bind(this));
    if (this.nextButton) this.nextButton.addEventListener('click', this.moveToNext.bind(this));
    this._onScrollEnd();
 ...
 */ function checkButtonState(sliderInner, prevButton, nextButton) {
    if (prevButton) {
        if (sliderInner.scrollLeft < 50) prevButton.setAttribute('disabled', true);
        else prevButton.removeAttribute('disabled');
    }
    const scrollMax = sliderInner.scrollWidth - sliderInner.clientWidth;
    if (nextButton) {
        if (sliderInner.scrollLeft >= scrollMax - 50) nextButton.setAttribute('disabled', true);
